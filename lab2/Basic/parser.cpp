...
Statement *parseStatement(string order, string source) {
	if (order == "PRINT") return new Print_statement(source);
	if (order == "LET") return new Let_statement(source);
	if (order == "INPUT") return new Input_statement(source);
	if (order == "GOTO") return new Goto_statement(source);
	if (order == "IF") return new If_statement(source);
	if (order == "REM") return new Rem_statement(source);
	if (order == "END") return new End_statement(source);
	error("SYNTAX ERROR");
	return NULL;
}